...
class Digraph(object):
    ...
    def add_edge(self, edge):
        ...
        in the  graph."""
        nodes_in_graph = (edge.get_source() in self.nodes and
                          edge.get_destination() in self.nodes)
        if nodes_in_graph:
            self.edges.setdefault(edge.get_source(),[]).append(edge)
            # Destination node shouldn't need to know about the endge
            #edges[edge.get_destination()].append(edge)
        else:
            raise ValueError
